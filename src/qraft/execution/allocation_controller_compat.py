"""Legacy SIESTA-shaped allocation-controller compatibility implementation.

This module preserves the accepted schema 1/2 package and recovery behavior.
New compiled workflows execute through :mod:`qraft.execution.capability_runtime`;
engine-specific semantics must not move back into that generic authority.
"""
...
def load_controller_config(path: Path) -> ControllerConfig:
    ...
    if account is not None:
        if not isinstance(account, str):
            raise ValueError("explicit configuration required: account")

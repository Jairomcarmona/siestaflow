...
from .models import ArtifactDescriptor, FDFBlock, FDFDocument, FDFScalar, normalize_label


_BOHR_TO_ANG = 0.529177210903
_RY_PER_BOHR_TO_EV_PER_ANG = 13.605693122994 / _BOHR_TO_ANG
_GEOMETRY_TYPE = "qraft.geometry"
_FORCE = re.compile(r"max(?:imum)?\s+force\s*[:=]\s*([-+0-9.Ee]+)\s*([A-Za-zÅ/]+)", re.I)
...
def geometry_from_fdf(path: Path) -> dict[str, Any]:
    ...
    fmt = (_scalar(document, "AtomicCoordinatesFormat").value if _scalar(document, "AtomicCoordinatesFormat") else "Ang").casefold()
    atoms = []
...
def _force_from_text(text: str) -> float | None:
    matches = list(_FORCE.finditer(text))
    if not matches:
        return None
    match = matches[-1]
...
def validate_relaxation(document: FDFDocument) -> float:
    ...
    if variable is not None and variable.value.strip().casefold() in {"t", "true", "yes", "1"}:
...
class SiestaRelaxationCapability:
    ...
    def validate_input(self, inspected: FDFDocument, **kwargs: Any):
        ...
        return self.base.validate_input(inspected, **kwargs)

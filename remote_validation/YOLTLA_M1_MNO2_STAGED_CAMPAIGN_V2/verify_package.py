...
def main() -> int:
    ...
    if (
        "launcher_from_config" not in controller
        or "ResourceManager" not in controller
        or "campaign_state.json" not in controller
    ):
    ...
    result = subprocess.run(
        [sys.executable, "scripts/campaignctl.py", "verify", "--with-external"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    ...
    print("YOLTLA_M1_V2_PACKAGE_VERIFIED")

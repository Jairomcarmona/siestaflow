...
def load_controller_config(path: Path) -> ControllerConfig:
    ...
    srun_raw = runtime.get("srun_command")
    if not isinstance(srun_raw, list) or not srun_raw:
        raise ValueError("runtime.srun_command must be a non-empty argument list")
    srun_command = tuple(_required_text(item, "runtime.srun_command") for item in srun_raw)
    srun_args_raw = runtime.get("srun_arguments", [])
...
class AllocationController:
    ...
    def __init__(
        ...
        *,
        ...
        launcher: SrunLauncher | None = None,
        ...
        self.config = config
        self.slurm = slurm
        self.launcher = launcher or SrunLauncher(
        ...
        return {
            ...
            "tasks": {
                task.task_id: {
                    ...
                    "last_attempt": None, "result_manifest_sha256": None, "reason": "not started",
                }
    ...
    def _execute(self, task: ControllerTask, attempt_id: str, attempt: Path, primary: Path) -> tuple[StepOutcome | None, str | None]:
        spec = StepLaunchSpec(
            task.task_id, attempt_id, attempt, primary, attempt / "stdout.txt", attempt / "stderr.txt",
            task.mpi_processes, task.cpus_per_process, self.config.siesta_executable,
            self.config.executable_arguments, self.config.environment,
        )
        try:
            outcome = self.launcher.launch(spec)
            return outcome, None
    ...
    def _write_attempt_evidence(self, task: ControllerTask, attempt: Path, outcome: StepOutcome) -> dict[str, Any]:
        ...
        manifest = {
            ...
            "normal_termination": record.normal_termination, "scf_started": record.scf_started,
            "scf_converged": record.scf_converged, "parser_classification": record.classification.value,
    ...
    def _validate_attempt(self, task: ControllerTask, attempt_id: str, expected_manifest_hash: str | None = None) -> tuple[bool, str]:
        ...
        if task.require_scf_converged and not manifest.get("scf_converged"):
    ...
    def run(self, *, install_signal_handlers: bool = True) -> ExecutionStatus:
        ...
        active: dict[Future[tuple[StepOutcome | None, str | None]], tuple[ControllerTask, str, Path]] = {}
        used_cpus = 0
                ...
                if not self.shutdown.requested:
                    for task in self.config.tasks:
                        ...
                        try:
                            ...
                            continue
                        attempted_this_allocation.add(task.task_id)
                        self._set_task(task.task_id, ExecutionStatus.RUNNING, "srun step launched", last_attempt=attempt_id)
                        future = executor.submit(self._execute, task, attempt_id, attempt, primary)
                        active[future] = (task, attempt_id, attempt)
                        used_cpus += task.cpus
                        launched = True
                if active:
                    ...
                    for future in done:
                        task, attempt_id, attempt = active.pop(future)
                        used_cpus -= task.cpus

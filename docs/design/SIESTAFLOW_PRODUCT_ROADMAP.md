...
| 4 — DAG adaptativo | `LOCAL_APPROVAL_PROPAGATION_VALIDATED` | reglas de `Mesh.Cutoff` y k-grid; productor real de observaciones; compositor de fragmentos con puertos científicos tipados; selección manual aislada/parcial/encadenada mediante API/CLI; relajación CG con FDF y PSML hash-bound validada localmente; aprobación/rechazo humano persistido; perfil numérico y receta `converge_then_relax` con evidencia hash-bound; cadena local completa de convergencia técnica → aprobación → relajación SIESTA real | recuperaciones autorizadas, consumo DOS/bandas/óptica y aceptación remota de un flujo completo; convertir reglas técnicas en campañas científicas con criterios y evidencia adecuados |

...
| 4 — DAG adaptativo | `LOCAL_APPROVAL_PROPAGATION_IMPLEMENTED` | reglas de `Mesh.Cutoff` y k-grid; productor real de observaciones; compositor de fragmentos con puertos científicos tipados; selección manual aislada/parcial/encadenada mediante API/CLI; relajación CG con FDF y PSML hash-bound validada localmente; aprobación/rechazo humano persistido; perfil numérico y receta `converge_then_relax` con evidencia hash-bound; ruta canónica preservada | ejecutar y validar localmente la cadena completa de convergencia aprobada → relajación, recuperaciones autorizadas, consumo DOS/bandas/óptica, y aceptación remota |

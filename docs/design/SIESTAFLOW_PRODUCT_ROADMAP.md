...
| 4 — DAG adaptativo | `LOCAL_TWO_AXIS_AUTHORING_VALIDATED` | reglas de `Mesh.Cutoff` y k-grid; registro explícito de builders/recetas; ambas recorren intención → `WorkflowDefinition` → lock → `run prepare` → paquete → evaluación local | productor de observaciones SIESTA, aprobación/propagación, `converge_then_relax`, capacidad consumidora DOS/bandas y aceptación remota |

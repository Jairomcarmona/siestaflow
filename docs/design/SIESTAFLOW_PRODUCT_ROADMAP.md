...
| 4 — DAG adaptativo | `LOCAL_AUTHORING_VERTICAL_SLICE` | regla general de `Mesh.Cutoff`; registro explícito de builders/recetas; intención → `WorkflowDefinition` → lock → `run prepare` → paquete → evaluación local de evidencia Mesh | productor de observaciones SIESTA, k-grid como segunda validación del contrato, aprobación/propagación, `converge_then_relax` y aceptación remota |

...
| 4 — DAG adaptativo | `LOCAL_SCIENTIFIC_RULE_CONTRACT` | corte canónico local hash-bound; regla general de convergencia de `Mesh.Cutoff` con unidades, estabilidad consecutiva, malla real, fuerzas, SCF, firma magnética, eggbox, extensión acotada y revisión humana | integrar la regla científica con `workflow.lock.json`/`run prepare`, `converge_then_relax` y aceptación remota |

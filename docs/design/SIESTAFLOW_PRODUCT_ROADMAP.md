...
| 4 — DAG adaptativo | `LOCAL_CANONICAL_ADAPTIVE_SLICE` | corte canónico local: tres variantes materializadas, selector determinista, fan-in y consumidor final hash-bound mediante `run prepare` | parámetros SIESTA autorizados, criterios con unidades/estabilidad, `converge_then_relax` y aceptación remota |

...
| 4 — DAG adaptativo | `LOCAL_COMPOSABLE_AUTHORING_FOUNDATION` | reglas de `Mesh.Cutoff` y k-grid; productor real de observaciones; compositor de fragmentos con puertos científicos tipados; selección manual aislada/parcial/encadenada mediante API/CLI; capacidad inicial de relajación CG con FDF y PSML hash-bound; ruta canónica preservada | aprobación/propagación, `converge_then_relax`, ejecución local de la relajación y consumo DOS/bandas/óptica, y aceptación remota |

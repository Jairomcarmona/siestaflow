...
| 4 — DAG adaptativo | `LOCAL_ELECTRONIC_ANALYSIS_INTEGRATION_VALIDATED` | convergencia y aprobación; relajación; DOS/PDOS, bandas y óptica aislados; receta CLI `ground_state → DM → {bands, dos_pdos, optics}` ejecutada 4/4 localmente; las tres transferencias y lecturas DM verificadas; exportaciones hash-bound sin interpretación | aceptación remota del flujo técnico completo; convertir reglas técnicas en campañas científicas con criterios y evidencia adecuados |

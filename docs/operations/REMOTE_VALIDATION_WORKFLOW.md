...
The implemented sequence is `LOCAL definition → PREVIEW package → human transfer/configuration/authorization → remote action outside Codex → hashed bundle → LOCAL conservative import`. SIESTAFLOW itself does not call SSH or submit jobs.

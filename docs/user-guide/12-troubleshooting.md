...
| Bloques FDF faltantes | El FDF no es entrada SIESTA suficiente | `qraft check`, species, lattice y coordenadas | No ignores `BLOCKED`. |
...
| Engine `BLOCKED` | SIESTA no es ejecutable o accesible | `--siesta`, permisos, `qraft setup env` | No asumas que `qraft inspect plan` garantiza disponibilidad en run. |
